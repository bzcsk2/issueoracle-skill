...
1. **GitHub Issue/PR Content**
   ...
   - _Mitigation_: `safety.py` sanitizes all untrusted content; bodies truncated to 2000 chars; dangerous patterns redacted
...
4. **Local Code Scanning**
   - IssueOracle reads local source files only
   - No code is transmitted externally without explicit opt-in
   - _Mitigation_: `ISSUEORACLE_ALLOW_REMOTE_LLM=0` by default; code stays local
...
Mining result (local only, no redistribution)
  ↓
Human review → approved → packs/ (version-controlled)
  ↓
